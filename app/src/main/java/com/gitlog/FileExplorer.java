...
import com.gitlog.adapter.FileAdapter;
...
public class FileExplorer extends AppCompatActivity implements FileAdapter.OnRepoListener {
    ...
    void decodeJson(String responseBody) {
        if (TextUtils.isEmpty(responseBody) || responseBody.length() < 50) {
            Toast.makeText(this, "Ошибка ответа JSON: " + responseBody, Toast.LENGTH_SHORT).show();
        }
        ...
        try {
            ...
            if (!repos.isEmpty()) {
                sortLinks(repos);
                //работает только в том случае, если мы изменяем набор данных;
                //если мы меняем набор данных, нам нужно создать новый адаптер
                adapter.notifyDataSetChanged();
            } else {
                Toast.makeText(this, "JSON не возвращает файлы!", Toast.LENGTH_SHORT).show();
            }
